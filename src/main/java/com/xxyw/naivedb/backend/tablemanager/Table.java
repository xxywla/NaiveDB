...
import com.xxyw.naivedb.backend.utils.ParseStringRes;
import com.xxyw.naivedb.backend.utils.Parser;
import com.xxyw.naivedb.common.MyError;
import com.xxyw.naivedb.backend.tablemanager.Field.ParseValueRes;
...
public class Table {
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        if (fd == null) {
            throw MyError.FieldNotFoundException;
        }
    ...
    private Map<String, Object> string2Entry(String[] values) throws Exception {
        if (values.length != fields.size()) {
            throw MyError.InvalidValuesException;
        }
        ...
        if (where == null) {
            ...
            for (Field field : fields) {
                if (field.fieldName.equals(where.singleExp1.field)) {
                    if (!field.isIndexed()) {
                        throw MyError.FieldNotIndexedException;
                    }
                    fd = field;
                    break;
                }
    ...
    private CalWhereRes calWhere(Field fd, Where where) throws Exception {
        ...
        switch (where.logicOp) {
            ...
            case "and":
                ...
                break;
            default:
                throw MyError.InvalidLogOpException;
        }
        return res;
    }
