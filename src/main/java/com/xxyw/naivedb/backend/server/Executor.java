package com.xxyw.naivedb.backend.server;

import com.xxyw.naivedb.backend.parser.Parser;
import com.xxyw.naivedb.backend.parser.statement.*;
import com.xxyw.naivedb.backend.tablemanager.BeginRes;
...
public class Executor {
    ...
    public byte[] execute(byte[] sql) throws Exception {
        ...
        if (Begin.class.isInstance(stat)) {
            if (xid != 0) {
                throw MyError.NestedTransactionException;
            }
            ...
            if (xid == 0) {
                throw MyError.NoTransactionException;
            }
